...
for (i in 1:1) {
  for (j in 2:2) {
    ...
    if(EXCLUDE_WEAK) {
      # calculate F-statistics for HDL
      # don't exclude here - exclude after adj etc
      F = cbind(data$LDL_beta^2/data$LDL_se^2, 
                data$HDL_beta^2/data$HDL_se^2, 
                data$TG_beta^2/data$TG_se^2)
      Fmax = data$HDL_beta^2/data$HDL_se^2 # rowMaxs(F) 
      if(length(Fmax[Fmax<11]) > 0) {
        data <- data[-match(Fmax[Fmax<11],Fmax), ]
      }
    }
