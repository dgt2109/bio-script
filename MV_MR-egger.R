...
#  subset <- c(which((data$HDL_beta/data$HDL_se)^2 >= Inf),
#              which((data$HDL_beta/data$HDL_se)^2 < 100))
#  data <- data[-subset, ]
...
#  data <- head(data,nvars)
  ...
  allres <- matrix(c(beta_ivw,se_ivw,exp(beta_ivw),exp(lb_ivw),exp(ub_ivw),
              p_ivw,i2h_ivw,theta1ME,se_theta1ME.random,exp(theta1ME),
  ...
  # Q_0.10 <- snpwise_Q[log10(F) < 1]
  # Q_10.100 <- snpwise_Q[abs(log10(F)-1.5) <= 0.5]
  # Q_100.Inf <- snpwise_Q[log10(F) > 2]
  # b1 <- c(Q_0.10,Q_10.100,Q_100.Inf)
  # b1lev <- c(rep(1,length(Q_0.10)),
  #            rep(2,length(Q_10.100)),
  #            rep(3,length(Q_100.Inf)))
  # hetP_0.10 <- snpwise_het[log10(F) < 1]
  # hetP_10.100 <- snpwise_het[abs(log10(F)-1.5) <= 0.5]
  # hetP_100.Inf <- snpwise_het[log10(F) > 2]
