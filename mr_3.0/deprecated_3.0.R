...
egger2 <- lm(CAD_beta ~ LDL_beta+HDL_beta+TG_beta, data=W, x=TRUE)
simex <- simex(model=egger2,
               SIMEXvariable=c("LDL_beta","HDL_beta","TG_beta","CAD_beta"),
               measurement.error=errors[[i]],
...
err <- cbind(data[le[i]],data[le[i%%3+1]],data[le[(i+1)%%3+1]],data["CAD_se"])
...
# dev.off()
# 
# # ideally put in a loop w/ different variations
